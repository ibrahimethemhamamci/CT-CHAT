        ...
        if images is not None and len(images) > 0 and self.is_multimodal:
            if len(images) > 0:
                if len(images) != prompt.count(DEFAULT_IMAGE_TOKEN):
                    ...
                    raise ValueError(f"Number of images does not match number of <image> tokens in prompt, {len(images)}, {prompt.count(DEFAULT_IMAGE_TOKEN)}")

                images = [np.load(image)["arr"] for image in images]
                ...
                images = None
                image_sizes = None
            image_args = {"images": images[0], "image_sizes": image_sizes}
        else:
            images = None
            image_args = {}
